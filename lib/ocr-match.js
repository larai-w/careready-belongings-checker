...
// 素の文字列と濁点除去後の文字列の両方でDice係数を計算し、高い方を採用する。
export function similarity(a, b) {
    const x = normalizeOcrText(a);
    const y = normalizeOcrText(b);
    if (!x || !y) return 0;
    if (x === y) return 1;
    let best = bigramDice(x, y);
    const xs = stripDakuten(x);
    const ys = stripDakuten(y);
    if (xs !== x || ys !== y) {
        if (xs === ys) return 1;
        best = Math.max(best, bigramDice(xs, ys));
    }
    return best;
}
...
export function inferOcrCategory(name, categories, hints = OCR_CATEGORY_HINTS) {
    ...
    // 2) ヒント語(カタログ外の名前だけ。旧実装と同じく定義順で先に合ったものを採用)
