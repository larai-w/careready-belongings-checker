...
クローズドパイロット前に、施設スタッフ用 Cognito 認証情報を安全化(ローテーション)し、サインインと復旧手順を検証する。旧パスワードが過去の AI 作業ログに残っている可能性があるため、これは **M1 安全確認**ブロッカーである(Issue #7)。
...
- [x] 新しい一意の恒久パスワードを chat・repo 履歴の外で設定した
- [x] メールでのブラウザサインインを検証した
- [x] runbook がメールのサインインエイリアスと Cognito 内部 Username を区別していることを確認した(本 runbook「前提」節)
- [x] パスワード等を保存せずに完了を記録した

完了日: **2026-08-01** / 実施者(GitHub 名): **larai-w**

検証結果: **メールエイリアスでの CLI 認証、実ブラウザでのサインイン、認証後のテンプレート一覧 API 読み込みに成功**。認証値は外部の安全な保管先で管理し、本記録には含めていない。
